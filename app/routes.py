...
def get_models():
    ...
    for model_path in model_files:
        ...
        try:
            ...
            models_data[filename] = {'name': os.path.splitext(filename)[0], 'size': f"{file_size_mb:.2f} MB"}
        except OSError as e:
...
def upload_model_file():
    """处理模型文件上传。"""
    if 'model' not in request.files: return jsonify({'error': '请求中缺少模型文件部分'}), 400
    file = request.files['model']
    if file.filename == '': return jsonify({'error': '未选择任何文件'}), 400
    if file:
...
def start_inference_thread():
    ...
    # --- 从 app.config 中获取新的控制器实例 ---
    mouse_controller = current_app.config.get('MOUSE_CONTROLLER')
    drawing_controller = current_app.config.get('DRAWING_CONTROLLER')
    scaling_factor = current_app.config.get('SCALING_FACTOR', 1.0)

    # --- 使用新的控制器进行健壮性检查 ---
    if not drawing_controller:
        return jsonify({'status': 'error', 'message': '绘图控制器不可用，无法启动。'}), 500
    if not mouse_controller or not mouse_controller.is_ready:
        return jsonify({'status': 'error', 'message': '鼠标控制器未就绪，无法启动推理。'}), 503

    try:
        # --- 使用新的控制器实例化 InferenceThread ---
...
def stop_inference_thread():
    ...
    try:
        ...
        inference_thread.join(timeout=5.0) # 等待线程结束
        inference_thread = None
